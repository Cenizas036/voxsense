...
MODEL_PATH = os.path.join(BASE_DIR, "gender_model.pth")
...
def collect_all_audio_files(root_dir):
    files = []
...
def main():
    ...
    all_files = collect_all_audio_files(DATASET_DIR)
    ...
    for file in all_files:
        ...
        if metadata and metadata["gender"] in ["male", "female"]:
            file_list.append(file)
            labels.append(0 if metadata["gender"] == "male" else 1)

    print("Total usable samples:", len(file_list))
    ...
    print("Gender model saved:", MODEL_PATH)
